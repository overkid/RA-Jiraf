    ...
    if (isAdminLoggedIn() && $pdo instanceof PDO) {
        ...
        if ($action === 'save_notification_email') {
            ...
            if ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                $errors[] = 'Укажите корректный email для уведомлений.';
            } else {
                ...
                $stmt->execute([':key' => 'notification_email', ':value' => $email]);
                $success = 'Email для уведомлений сохранён.';
            }
        }
    ...
    <main class="admin-layout">
      ...
      <?php else: ?>
        ...
        <section class="admin-card">
          ...
          <p class="admin-muted">Укажите email, на который будут приходить уведомления о новых заявках.</p>
          <form method="post" class="admin-form-grid">
            <input type="hidden" name="action" value="save_notification_email" />
            <label>Email для уведомлений
              <input type="email" name="notification_email" value="<?= htmlspecialchars($content['notification_email'] ?? '', ENT_QUOTES, 'UTF-8') ?>" placeholder="example@mail.com" />
            </label>
            <button type="submit" class="btn btn-nav">Сохранить email</button>
          </form>
        </section>
